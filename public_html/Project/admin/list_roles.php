<!-- if (!has_role("Admin")) { // makes sure the user is a admin

    we can concate the query before execution to get a more complex query

    limit -> only allows a certain amount of replies 

    second form:  each row will have a seperate form for each role -> submit with toggle allows to enable or disable roles 
-->
...
if (isset($_POST["role"])) {
    ...
    $params =  [":role" => "%$search%"];
    // contains match --> use % as any character
}
...
<form method="POST">
    ...
    <!-- allows us search for specific role -> keeps data around if reloading -->
        ...
        <?php else : ?>
            <?php foreach ($roles as $role) : ?>
                <tr>
                    ...
                    <!-- ternary to conver to true or false -->
                    <td>
                        <!-- each row will have a seperate form for each role -> submit with toggle allows to enable or disable roles -->
                        <form method="POST">
